...
static void waitingListLean(waiting_list_stats *wl_stats, char *policy){
    ...
    for (i = 0; i < NUM_BLOOD_TYPES; ++i) {
        for (j = 0; j < NUM_PRIORITIES; ++j) {
            ...
            OPEN_FILE(f_wl, path)
            fprintf(f_wl, format,
                    wl_stats->sum_patient_arrivals[i][j],
                    wl_stats->sum_patient_served[i][j],
                    wl_stats->sum_patient_reneges[i][j] + wl_stats->sum_patient_deaths[i][j],
...
static void organBankLean(organ_bank_stats *ob_stats, char *policy){
    ...
    for (i = 0; i < NUM_BLOOD_TYPES; ++i) {
        ...
        OPEN_FILE(f_ob, path)
        fprintf(f_ob, format,
                ob_stats->sum_organ_arrivals[i][deceased],
                ob_stats->sum_organs_completions[i][deceased],
                ob_stats->sum_organ_outdatings[i],
...
static void transplantLean(transplant_stats *t_stats, char *policy){
    ...
    for (i = 0; i < NUM_BLOOD_TYPES; ++i) {
        for (j = 0; j < NUM_PRIORITIES; ++j) {
            ...
            OPEN_FILE(f_tr, path)
            fprintf(f_tr, format,
                    t_stats->sum_arrivals[i][j],
                    t_stats->sum_transplanted[i][j],
                    t_stats->sum_rejected[i][j],
...
static void activationLean(activation_stats *a_stats, char *policy){
    ...
    for (i = 0; i < NUM_BLOOD_TYPES; ++i) {
        ...
        fprintf(f_act, format,
                a_stats->sum_arrivals[i],
                a_stats->sum_activated[i],
                a_stats->sum_reneges[i] + a_stats->sum_deaths[i],
                a_stats->avg_wait[i],
                a_stats->avg_delay[i],
